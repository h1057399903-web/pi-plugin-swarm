...
}, { maxConcurrency: 16, initialLaunchLimit: 5, launchStaggerMs: 700, maxRetries: 0, timeoutMs: 190_000 });
...
const summary = { peakActive, wallMs: Date.now() - wallStart, completed: workers.filter((worker) => worker.status === "completed").length, exact: workers.filter((worker) => worker.output === `SWARM16_OK_${worker.index}`).length, workers };
