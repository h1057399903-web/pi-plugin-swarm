...
const batch = new SwarmBatch(Array.from({ length: 16 }, (_, i) => i), async (i, context) => {
  ...
  try {
    return await runtime.run({ workerId: `abort16-${i}`, cwd: "/tmp", timeoutMs: 120_000, prompt: "Use bash to run sleep 60, then reply DONE." }, () => { runtimeProgressEvents++; });
  } finally {
    context.signal.removeEventListener("abort", stop);
    active--;
    cleaned++;
  }
}, { maxConcurrency: 16, initialLaunchLimit: 5, launchStaggerMs: 700, signal: controller.signal, onProgress: () => { batchProgressEvents++; } });

const promise = batch.run();
setTimeout(() => controller.abort(new Error("acceptance abort")), 8_500);
const results = await promise;
const summary = { peakActive, active, cleaned, wallMs: Date.now() - startedAt, eventCount: runtimeProgressEvents + batchProgressEvents, runtimeProgressEvents, batchProgressEvents, statuses: results.map((result) => result.status) };
