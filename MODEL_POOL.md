...
- When the file is valid, it becomes the Swarm whitelist. The coordinator sees only aliases, cost classes, descriptions, plus the reserved `primary`; it may pass `model: "alias"`, use `model: "primary"` for hard work, or omit `model` to use `defaultModel`.
...
- The public run/worker model label is the alias (or `primary`); the concrete target is used only to start the worker.

- 文件不存在时，原来的 `/swarm model` 行为完全不变。
- 文件有效时，它就是 Swarm 白名单。主模型只看到 alias、费用类别、描述以及保留的 `primary`；单次 run 可传 `model: "alias"`，困难任务可用 `model: "primary"`，省略则使用 `defaultModel`。
...
- 公共 run/worker 只显示 alias（或 `primary`）；真实 target 只用于本机启动 worker。
